...
export const fetchUserList = async (
  setData: React.Dispatch<React.SetStateAction<institutionProps[]>>
) => {
  apiGateway
    .get("/api/v1/yip/list-users/")
    .then((res) => res.data.response)
    .then((data) => setData(data))
    .catch((err) => console.log(err));
};
export const fetchLegislativeAssemblies = async (
  setData: React.Dispatch<React.SetStateAction<institutionProps[]>>
) => {
  apiGateway
    .get("/api/v1/yip/list-legislative-assembly/")
    .then((res) => res.data.response)
    .then((data) => setData(data))
    .catch((err) => console.log(err));
};
...
export const addUser = async (postData: postDataUserProps) => {
  apiGateway
    .post("/api/v1/yip/create-user/", postData)
...
export const handleDelete = (
  ...
  apiGateway
    .delete(
      `/api/v1/yip/delete-${
        getCurrentPageUtils().content !== "Users" ? "model-schools" : "user"
      }/${schoolId}/`
    )
...
export const getRoles = async (
  ...
  apiGateway
    .get("/api/v1/yip/get-roles/")
    .then((res) => res.data.response.club_status)
    .then((data) => {
      yip.roles = data.map((role: string, index: number) => ({
        value: index,
        label: role,
      }));

      setRoles(
        data.map((role: string, index: number) => ({
          value: index,
          label: role,
        }))
      );
